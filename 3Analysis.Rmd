...
#setup
knitr::opts_chunk$set(echo = FALSE, message = F) #echo enables
...
t <- adata_amr %>% group_by(AMR) %>% summarise(n = n())
```
...
```{r}
t <- adata_each %>% group_by(AMR) %>% summarise(mean_entropy = mean(entropy, na.rm = T))
...
p <- ggplot(data = adata_occ, aes(x = log(bevoelkerung, base = 10))) +
...
### 1.2.2 Entropy X AMR
```{r Entropy x AMR plot, fig.height=8, fig.width=8}
#beta = 0.00601668639832804, scales overproportionally, super small again -> how to interpret?
...
```{r, Entropy x AMR beta}

model1 <- lm(log(mean_entropy) ~ log(bevoelkerung), data = amr_entro)
summary(model1)


# beta = 0.006, super significant, 10% increase in population 0.06 % more information
```
...
### 2.2.2 Entropy diff Plot

```{r combine boots and plot CIs, fig.height=7, fig.width=7, echo = FALSE}
# wo sind die errorbars
...
Two plots which show that industries differ
### 2.3.1 IT vs. Food
```{r, scaling differences, IT vs. Food}
...
### 2.3.1 Raw materials processing, glass,\n ceramics, plastics, wood vs. Food
```{r, scaling differences, IT vs. Food}
...
```{r facet intra industry amr, fig.height=10, fig.width=10, echo = FALSE}
...
test <- adata_occ %>% dplyr::filter(occupation_major == "Skilled Agricultural, Forestry and Fishery Workers")
```
...
## 4.1.2 Creative vs. non-creative
...
model_c <- lm(log(mean_entropy) ~ log(population), data = creatives)
print(paste("creatives beta = ", model_c$coefficients[2]))


model_nc <- lm(log(mean_entropy) ~ log(population), data = non_creatives)
print(paste("non-creatives beta = ", model_nc$coefficients[2]))
