...
```{r}
# exclude cases where ID = next(ID) AND creative = next(creative) -> excludes duplicates
# 2631 cases where baa description doesn't overlap with isco classification so that jobs are sorted in two categories: creative and non-creative

adata_occ_pruned <- adata_occ %>%
  dplyr::filter(!(ID == lead(ID) & creative == lead(creative)))


sample_occ <- adata_occ_pruned %>%
  slice(1:100) %>%
  select(ID, Titel, Beruf, baa_key, isco_key, creative) %>%
  glimpse()


pruned <- adata_occ_pruned %>% select(-baa_key,-baa_bezeichnung, -baa_bezeichnung_eng, -creative, -occupation_major, -occupation_submajor, -isco_key, -unit_group_eng)

each <- adata_each 

test <- distinct(pruned)
