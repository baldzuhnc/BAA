...
```{r, include = FALSE}
gummibaerchen <- adata_each %>% dplyr::filter(200 > str_length(Stellenbeschreibung)) %>% slice(371:371)
gummibaerchen
```
...
```{#r bootstrap CIs entropy differences, include=FALSE}
library(boot)
...
```{r, plot entropy differences, fig.height=6, fig.width=6, echo = FALSE}
...
```{r scaling x entropy, fig.height=6, fig.width=6, echo = FALSE}
options(ggrepel.max.overlaps = Inf)
...
# 4.2 Scaling differences

## 4.2 Micro: Creative vs. non-creative scaling differences
...
Both creative an non-creative scale overproportionally. Scaling coefficients differ between occupation types (complex vs. non creative).  
...
## 4.3 Micro: Major groups scaling differences
...
```{r plot ajor differences bootstrap CIs, fig.height=6, fig.width=10, echo = FALSE}
...
## 5.1 All Ads
```{r, map of all jobads, fig.height=6, fig.width=6, echo = FALSE}
