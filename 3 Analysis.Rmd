...
```{r open data}
# renaming kann auch weg wenn erstmal complete ####
industries_rename <- function(adata){
...
```{r architects}
archi <- adata_amr %>% dplyr::filter(industry == "Construction, Architecture")
```
...
pop_entropy <- ggplot(data = adata_each, aes(x = log(bevoelkerung, base = 10), y = entropy)) +
...
entropy_industry <- ggplot(data = industry_data, aes(x = mean_entropy, y = beta)) +
  ...
  labs(x = "Average entropy of job ads", y = "Scaling coefficient (β)") +
  theme_minimal()
entropy_industry
```
...
#Architekturbranche
archi <- adata_each %>% 
  dplyr::filter(industry == "Vehicle construction, vehicle maintenance") %>% 
  group_by(AMR) %>% 
  summarise(mean_entropy = mean(entropy, na.rm = T),
            population = mean(bevoelkerung))

model2 <- lm(formula = log(mean_entropy) ~ log(population),
                        data = archi)
